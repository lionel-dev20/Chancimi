...
import dataquizchap12 from '../dataquizchap12';
export default function Quiz() {
  ...
  const handleAnswer = (answer) => {
    ...
    const correct = dataquizchap12[currentQuestion].correctAnswer === answer;
    if (correct) {
      setScore(score + 1);
    }

    setUserAnswers([...userAnswers, { question: dataquizchap12[currentQuestion].question, selected: answer, correct }]);

    // Move to next question
    if (currentQuestion + 1 < dataquizchap12.length) {
  ...
  const finalScore = (score / dataquizchap12.length) * 100;
    ...
    <div className='grid grid-cols-1 p-3 sm:p-10 gap-5 bg-white m-[0.5rem] sm:m-10 rounded-[8px]'>
      {timeLeft > 0 && !quizFinished && (
        <div className='w-full'>
          <h2 className='text-2xl font-bold pb-6 text-gray-800'>Quiz sur la leçon: Le secourisme</h2>
          ...
          <h3 className='border-b-2 border-gray-100 text-xl font-bold text-gray-700 my-6'>Question {currentQuestion + 1} sur {dataquizchap12.length}</h3>
          <p className='sm:text-[20px]  text-md font-semibold text-gray-700 py-4' >{dataquizchap12[currentQuestion].question}</p>
          <div className='w-full'> 
            {dataquizchap12[currentQuestion].options.map((option, index) => (
