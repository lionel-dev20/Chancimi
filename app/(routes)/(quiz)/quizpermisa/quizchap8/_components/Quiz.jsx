...
import dataquizchap8 from '../dataquizchap8';
export default function Quiz() {
  ...
  const handleAnswer = (answer) => {
    ...
    const correct = dataquizchap8[currentQuestion].correctAnswer === answer;
    if (correct) {
      setScore(score + 1);
    }

    setUserAnswers([...userAnswers, { question: dataquizchap8[currentQuestion].question, selected: answer, correct }]);

    // Move to next question
    if (currentQuestion + 1 < dataquizchap8.length) {
  ...
  const finalScore = (score / dataquizchap8.length) * 100;
    ...
    <div className='grid grid-cols-1 p-3 sm:p-10 gap-5 bg-white m-[0.5rem] sm:m-10 rounded-[8px]'>
      {timeLeft > 0 && !quizFinished && (
        <div className='w-full'>
          ...
          <h3 className='border-b-2 border-gray-100 text-xl font-bold text-gray-700 my-6'>Question {currentQuestion + 1} sur {dataquizchap8.length}</h3>
          <p className='sm:text-[20px]  text-md font-semibold text-gray-700 py-4' >{dataquizchap8[currentQuestion].question}</p>
          <div className='w-full'> 
            {dataquizchap8[currentQuestion].options.map((option, index) => (
